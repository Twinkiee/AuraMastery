...
function AuraMastery:ProcessBuffs(buffs, target)
	for idx, buff in pairs(buffs.arBeneficial) do
		...
		if self.buffWatch["Buff"][target][buff.splEffect:GetId()] ~= nil then
			for _, watcher in pairs(self.buffWatch["Buff"][target][buff.splEffect:GetId()]) do
				watcher(buff)
			end
		end
	end
	...
	for idx, buff in pairs(buffs.arHarmful) do
		...
		if self.buffWatch["Debuff"][target][tostring(buff.splEffect:GetId())] ~= nil then
			for _, watcher in pairs(self.buffWatch["Debuff"][target][tostring(buff.splEffect:GetId())]) do
				watcher(buff)
			end
		end
	end
		...
		for k, v in pairs(abilities) do
			if v.bIsActive and v.nCurrentTier and v.tTiers then
				...
				if tier then
					...
					if self.buffWatch["Cooldown"][s:GetId()] ~= nil then
						for _, watcher in pairs(self.buffWatch["Cooldown"][s:GetId()]) do
							watcher(s)
						end
					end
				end
